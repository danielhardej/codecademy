# In this lesson we look at how do we choose the number of hidden layers and the number of units per layer? 
# That is a tough question and there is no good answer. The rule of thumb is to start with one hidden layer
# and add as many units as we have features in the dataset. However, this might not always work. We need
# to try things out and observe our learning curve.
...
def more_complex_model(X, learning_rate):
    ...
    model.add(input)
    model.add(layers.Dense(8, activation='relu'))       # Test different numbers of layers/hidden units in the model
