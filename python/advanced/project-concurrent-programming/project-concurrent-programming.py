# In this project, we are calculating the average of three lists using four different approaches:
# sequential approach
# async approach
# threading approach
# multiprocessing approach
# our goal will be to make this calculation as efficient as possible and
# see which approaches work well and which ones struggle.
...
def main_multiprocessing(list1, list2, list3):  # Main wrapper for multiprocessing example
  ...
  processes = []
  for i in range(len(lists)):
    p = multiprocessing.Process(target=cal_average, args=(lists[i],))
    processes.append(p)
    p.start()
